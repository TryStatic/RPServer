...
namespace RPServer.Util
{
    internal sealed class Logger
    {
        ...
        private Logger()
        {
            ...
            FileWrite(MySqlLogFn,
                $"------------------------ [SERVER STARTED AT: {DateTime.Now}] ------------------------");
            FileWrite(CommandLogFn,
                $"------------------------ [SERVER STARTED AT: {DateTime.Now}] ------------------------");
            FileWrite(AuthLogFn,
                $"------------------------ [SERVER STARTED AT: {DateTime.Now}] ------------------------");
            FileWrite(ChatLogFn,
        ...
        public void MySqlError(string errorMsg, uint errorCode)
        {
            NAPI.Util.ConsoleOutput(
                $"[{DateTime.Now:MM/dd/yyyy HH:mm:ss}][MySQL ERROR]: {errorMsg} [Code={errorCode}]");
            FileWriteAsync(MySqlLogFn,
                $"[{DateTime.Now:MM/dd/yyyy HH:mm:ss}][MySQL ERROR]: {errorMsg} [Code={errorCode}]");
        }
        ...
        private static async void FileWriteAsync(string file, string message)
        {
            ...
            using (var stream =
                new FileStream(filePath, FileMode.Append, FileAccess.Write, FileShare.Write, 4096, true))
            {
