...
namespace RPServer.Database
{
    internal static class DbConnectionProvider
    {
        ...
        public static async Task<bool> TestConnection()
        {
            using (var dbConn = CreateDbConnection())
            {
                ...
                do
                {
                    tries++;
                    Logger.GetInstance()
                        .SqlInfo($"Trying to establish a connection with the MySQL Database... ({tries})");
                    try
                    {
        ...
        {
            if (ProviderName == DbProvider.MySql) return MySqlClientFactory.Instance;
        ...
        {
            private DbProvider(string value)
            {
            ...
            public static implicit operator string(DbProvider provider)
            {
