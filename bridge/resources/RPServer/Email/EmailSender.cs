...
namespace RPServer.Email
{
    internal class EmailSender
    {
        ...
        {
            var message = $"<h2>Hello {token.Account.Username}!</h2><p>Here you have your security code to authenticate the ownership of your account:</p>" +
                          $"<h3>{token.Token}</h3>" +
                          ...
                          $"<p>Thanks, {Game.Globals.SERVER_NAME}</p>";

            await SendMailMessageAsync(token.EmailAddress, $"{Game.Globals.SERVER_NAME} - Email Verification", message);
        }
